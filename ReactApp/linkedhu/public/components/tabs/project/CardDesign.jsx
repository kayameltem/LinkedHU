...
const CardDesign = (props) => {
  ...
  const applyAnnouncement = (e) => {
	...
	const editAnnouncement = (e) => {
		...
		const announcementData = {'project': {'announcementId': props?.data?.announcementId}, 'endDate': props?.data?.endDate};
		if(props.data.announcementId !== null || props.data.announcementId !== undefined)
		{
			const data = axios.post('http://localhost:8080/project/updateProject', announcementData);
		}
	}
	...
	const deleteAnnouncement = (e) => {
		...
		const announcementData = {'project': {'announcementId': props?.data?.announcementId}};
		console.log(announcementData);
		if(props.data.announcementId !== null || props.data.announcementId !== undefined)
		{
			const data = axios.post('http://localhost:8080/project/deleteProject', announcementData);
			setDeleted(true);
		}
	}
      ...
      <Modal show={show} fullscreen={true} onHide={handleClose}>
        ...
        <div className="row mt-5">
          <div className="col-lg-10 ms-5">
            ...
            <p><b>Project Type:</b> {props.data.type}</p>
            <p><b>Supervisor:</b> {props.data.supervisor}</p>
			...
			: <></>}
			{props.userType === 'UNDERGRADUATE' && props.data.owner !== props.userEmail ?
			<div><Button onClick={applyAnnouncement} className='mx-2' variant="primary" style={{ height: "3rem", background: "#008080" }}>
			Apply As Teammate
			</Button> 	{applied ? <div className = "pt-5"><Alert>Application is successfully completed.</Alert> </div>: <></>}</div>
			: <></>}
			{props.userType === 'ACADEMICIAN' && props.data.owner !== props.userEmail ?
			(<div><Button onClick={applyAnnouncement} className='mx-2' variant="primary" style={{ height: "3rem", background: "#008080" }}>
			Apply As Supervisor
			</Button>
