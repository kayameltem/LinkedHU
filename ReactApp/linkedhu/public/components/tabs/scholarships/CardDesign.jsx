...
const CardDesign = (props) => {
  ...
  const applyAnnouncement = (e) => {
    ...
    const announcementData = {
      application: { announcementId: props?.data?.announcementId },
      mail: props?.userEmail,
    };
    if (
      props.data.announcementId !== null ||
      props.data.announcementId !== undefined
    ) {
      const data = axios.post(
        "http://localhost:8080/announcements/apply",
        announcementData
      );
      setApplied(true);
    }
  ...
  const editAnnouncement = (e) => {
    ...
    const announcementData = {
      scholarship: { announcementId: props?.data?.announcementId },
    ...
    if (
      ...
      const data = axios.post(
        "http://localhost:8080/scholarship/updateScholarship",
        announcementData
      );
    }
  };
  ...
  const deleteAnnouncement = (e) => {
    ...
    if (
      ...
      const data = axios.post(
        "http://localhost:8080/scholarship/deleteScholarship",
        announcementData
      );
      setDeleted(true);
    }
        ...
        <Modal.Body>
          <div className="row mt-5">
            <div className="col-lg-10 ms-5">
              ...
              <p>
                <b>Scholarship Type:</b> {props.data.type}
              </p>
              <p>
                <b>Scholarship Amount: </b> {props.data.worth}
              </p>
              <p>
                <b>Start Date:</b> {props.data.startDate}
              </p>
              <p>
                <b>Due Date:</b> {props.data.endDate}
              </p>
              {props.data.owner === props.userEmail ? (
                <div>
                  ...
                  <Button
                    ...
                    className="mx-2"
                    style={{ height: "3rem", background: "#008080" }}
                  >
                    Show Applicants
                  </Button>
              ...
              {props.userType === "UNDERGRADUATE" &&
              props.data.owner !== props.userEmail ? (<div><Button onClick={applyAnnouncement} className='mx-2' variant="primary" style={{ height: "3rem", background: "#008080" }}>
              Apply
              </Button>
